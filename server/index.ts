...
⚠️ NOTE: Use REPLY only to:
- Summarize what you displayed (overview, NOT the actual content)
- Suggest next steps or follow-up questions
- Do NOT use REPLY to answer the user's query - all answers must be displayed via DISPLAY agent
...
4. DISPLAY: Render your findings using card, table, or chart (ALL answers go here)
5. REPLY: Brief overview of what was displayed + suggest next steps (NOT the answer itself)
...
2. Use the right prefix:
   ...
   - DISPLAY: for tables, charts, cards on canvas (display ALL answers here)
   - REPLY: overview of what was displayed + next steps (NOT the answer)
  ...
  while ((multiMatch = multiLinePattern.exec(response)) !== null) {
    ...
    if (cleanedArgs.startsWith('"') || cleanedArgs.startsWith("'")) {
      ...
      for (let i = 0; i < cleanedArgs.length; i++) {
        ...
        const prevChar = i > 0 ? cleanedArgs[i - 1] : '';
...
Syntax: \`card(title: "Card Title", instructions: "what to write about", variable: variable_name)\`
- title: REQUIRED - The card's title (will be displayed as header)
- instructions: REQUIRED - What the card content should convey
- variable: OPTIONAL - A variable containing text/insights to use (e.g., analysis answers)

Returns: Confirmation that card was displayed

Examples:

With variable (recommended when you have analysis results):
\`card(title: "Traffic Summary", instructions: "Summarize the key findings", variable: traffic_analysis)\`

Without variable (for general content):
\`card(title: "Welcome", instructions: "Write a brief welcome message about this dashboard")\`
...
// Build the Card Writer LLM prompt
function buildCardWriterPrompt(
  title: string,
  instructions: string,
  variableContent: string | null
): string {
  return `# CARD WRITER

You write concise, well-formatted markdown content for a card.

================================================================================
## CARD TITLE (Already set - DO NOT REPEAT)
================================================================================

"${title}"

IMPORTANT: The title above will be displayed as the card header. 
DO NOT include this title or any # heading with this title in your content.
Start your content directly with the information.
...
${variableContent ? `================================================================================
## VARIABLE DATA (Use this information)
================================================================================
...
Write ONLY the markdown content for the card body.

Rules:
1. DO NOT start with the title "${title}" - it's already the card header
2. You can use ## or ### for sub-sections within the card, but NOT for the main title
3. Use markdown formatting: **bold**, *italic*, bullet lists, numbered lists
4. Be concise and informative
5. If variable data is provided, incorporate the key information from it
6. If instructions say to display content EXACTLY - copy it verbatim, preserving all formatting

Output your markdown content now:`;
}
...
async function runDisplayAgent(
  ...
  try {
    ...
    // ========================================================================
    // Execute the display tool
    // ========================================================================
    
    if (call.toolName === 'card') {
      // Special handling for card - use Card Writer LLM
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🃏 DISPLAY AGENT CARD TOOL (with Card Writer LLM)`);
      console.log(`   call.args type: ${typeof call.args}`);
      console.log(`   call.args value: ${JSON.stringify(call.args).slice(0, 300)}`);
      console.log(`${'='.repeat(60)}\n`);
      
      // Parse card args: title, instructions, variable (optional)
      let cardTitle = 'Card';
      let cardInstructions = '';
      let cardVariableName: string | null = null;
      
      if (typeof call.args === 'object' && call.args !== null) {
        cardTitle = call.args.title || 'Card';
        cardInstructions = call.args.instructions || '';
        cardVariableName = call.args.variable || null;
      } else if (typeof call.args === 'string') {
        // Parse from string: title: "X", instructions: "Y", variable: Z
        const titleMatch = call.args.match(/title\s*:\s*["']([^"']+)["']/i);
        if (titleMatch) cardTitle = titleMatch[1];
        
        const instructionsMatch = call.args.match(/instructions\s*:\s*["']([^"']+)["']/i);
        if (instructionsMatch) cardInstructions = instructionsMatch[1];
        
        const variableMatch = call.args.match(/variable\s*:\s*([a-zA-Z_][a-zA-Z0-9_]*)/i);
        if (variableMatch) cardVariableName = variableMatch[1];
      }
      
      console.log(`   📋 Card Title: "${cardTitle}"`);
      console.log(`   📝 Instructions: "${cardInstructions}"`);
      console.log(`   📦 Variable: ${cardVariableName || '(none)'}`);
      
      if (!cardInstructions) {
        console.log(`   ❌ ERROR: No instructions provided for card`);
        sendEvent({ type: 'display_agent_error', error: 'Card requires instructions' });
        return { success: false, report: 'Card requires instructions parameter' };
      }
      
      // Detect if this is explicit content that should be displayed as-is
      // Indicators: multi-line content, poetry markers (*), or looks like pre-written text
      const hasNewlines = cardInstructions.includes('\n');
      const hasPoetryMarkers = cardInstructions.includes('*') && cardInstructions.split('*').length > 2;
      const looksLikeExplicitContent = hasNewlines || hasPoetryMarkers || cardInstructions.length > 200;
      
      let finalInstructions = cardInstructions;
      if (looksLikeExplicitContent) {
        finalInstructions = `IMPORTANT: Display the following content EXACTLY as provided, preserving all formatting, line breaks, and text. Do not modify, interpret, or rewrite it:\n\n${cardInstructions}`;
        console.log(`   ⚠️ Detected explicit content - instructing Card Writer to display as-is`);
      }
      
      // Get variable content if specified
      let variableContent: string | null = null;
      if (cardVariableName) {
        const pilotVar = variables.get(cardVariableName);
        if (pilotVar) {
          // Check if it has answers or text content
          if (pilotVar.actualData) {
            if (typeof pilotVar.actualData === 'string') {
              variableContent = pilotVar.actualData;
            } else if (pilotVar.actualData.answers) {
              variableContent = pilotVar.actualData.answers;
            } else {
              variableContent = JSON.stringify(pilotVar.actualData, null, 2);
            }
          }
          console.log(`   📄 Variable Content Preview: ${(variableContent || '').slice(0, 200)}...`);
        } else {
          console.log(`   ⚠️ Variable "${cardVariableName}" not found`);
        }
      }
      
      // Call Card Writer LLM
      console.log(`\n   🤖 Calling Card Writer LLM...`);
      sendEvent({ type: 'card_writer_calling', title: cardTitle, instructions: finalInstructions });
      
      const cardWriterPrompt = buildCardWriterPrompt(cardTitle, finalInstructions, variableContent);
      
      const cardWriterResponse = await retryLLMCall(
        () => client.chat.completions.create({
          model,
          messages: [
            { role: 'system', content: cardWriterPrompt },
            { role: 'user', content: 'Write the card content now:' }
          ],
          temperature: 0.4
        }),
        (res) => res.choices[0]?.message?.content || '',
      ...
      let cardContent = cardWriterResponse.choices[0]?.message?.content || '';
      
      console.log(`\n   📝 CARD WRITER OUTPUT:\n${cardContent}\n`);
      sendEvent({ type: 'card_writer_response', content: cardContent.slice(0, 300) });
      
      // Remove any leading title that matches the card title (in case LLM added it anyway)
      ...
      for (const pattern of titlePatterns) {
        cardContent = cardContent.replace(pattern, '');
      }
      
      // Also remove any title at the very start that looks like a heading
      cardContent = cardContent.trim();
      
      // Generate DSL for card
      const escapedContent = cardContent.replace(/"/g, '\\"').replace(/\n/g, '\\n');
      const dsl = `card: (title: "${cardTitle}", content: "${escapedContent}")`;
      
      generatedDSL.push(dsl);
      report = `Created card "${cardTitle}"`;
      
      console.log(`   ✅ Card DSL generated`);
      sendEvent({
        type: 'display_agent_tool_success',
        tool: 'card',
        title: cardTitle,
        message: 'Card displayed to user'
      });
    }
    else if (['line-chart', 'table', 'alert'].includes(call.toolName)) {
      // Use executeUITool for table, line-chart, alert (same as Executor)
      console.log(`\n${'='.repeat(60)}`);
